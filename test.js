...
function reloadLabirint() {
    ...
    if ((n >= 2) && (n <= 100) && (n >= 2) && (n <= 100)) {
        ...
        alert("Pease enter valid numbers 2-100");
    }
}
...
function findPathWork() {
    if (path_found) {
        sj = n - 1;
        si = 0;
        corrAttribute = attribute;
        ctx.fillStyle = 'green';
                ...
                if (i != 0)
                    if (!isBottomEdge(i - 1, j, ctx)) {
                        if (arrayLabirint[i - 1][j] == -1) {
                            ...
                            ctx.fillRect(j * 22 + 4, (i - 1) * 22 + 4, 14, 14);
                            ctx.fillStyle = 'grey';
                            ctx.fillText(attribute, j * 22 + 10, (i - 1) * 22 + 16);
                        }
                ...
                if (j != 0)
                    if (!isRightEdge(i, j - 1, ctx)) {
                        if (arrayLabirint[i][j - 1] == -1) {
                            ...
                            ctx.fillStyle = 'yellow';
                            ctx.fillRect((j - 1) * 22 + 4, (i) * 22 + 4, 14, 14);
                            ctx.fillStyle = 'grey';
                            ctx.fillText(attribute, (j - 1) * 22 + 10, (i) * 22 + 16);
                        }
...
//Рисуем путь по построенному массиву Ли (2,5 часа на анимацию)
function paintPath() {
    if (corrAttribute == 0) {
...
function cutLabirint(sx, sy, fx, fy, ctx) {
    var nx = (fx - sx) / 22;
    var ny = (fy - sy) / 22;
    if ((nx > 1) && (ny > 1)) {
        ctx.lineWidth = 2;
